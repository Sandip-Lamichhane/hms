...
const OpdTooltip = ({ active, payload, label }) => {
  if (active && payload?.length) {
    return (
      <div className="bg-slate-800 border border-[#c8eedc] rounded-xl p-3 shadow-xl">
  ...
  return (
    <ResponsiveContainer width="100%" height={240}>
      <LineChart data={PERF_MOCK} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
        ...
        <Legend formatter={(v) => <span className="text-slate-500 text-xs">{v}</span>} />
...
function KpiCard({ icon: Icon, label, value, sub, color }) {
  const colors = {
    indigo: 'text-emerald-700 bg-indigo-500/10',
    emerald: 'text-emerald-400 bg-emerald-500/10',
    amber: 'text-amber-400 bg-amber-500/10',
    rose: 'text-rose-400 bg-rose-500/10',
  };
  return (
    <div className="bg-white border border-[#c8eedc] rounded-2xl p-5 flex items-center gap-4">
  ...
  return (
    <SuperAdminLayout>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="bg-white border border-[#c8eedc] rounded-2xl p-6">
